...
from sycws_folder import main_body
from sycws_folder import model as model_r
...
def add_arguments(parser):
    ...
    # data
    parser.add_argument("--train_prefix", type=str, default='./data/cws-v2-data/train',
                        help="Train prefix, expect files with .txt/.lb suffixes.")
    parser.add_argument("--eval_prefix", type=str, default='./data/cws-v2-data/eval',
                        help="Eval prefix, expect files with .txt/.lb suffixes.")
    parser.add_argument("--eval_gold_file", type=str, default='./data/cws-v2-data/eval_gold',
                        help="Eval gold file.")

    parser.add_argument("--vocab_file", type=str, default='./data/cws-v2-data/vocab.txt',
                        help="Vocabulary file.")
    parser.add_argument("--embed_file", type=str, default='./data/cws-v2-data/char_vec.txt', help="""\
      Pretrained embedding files, The expecting files should be Glove formated txt files.\
      """)
    parser.add_argument("--index_file", type=str, default="./sycws_folder/indices.txt",
                        help="Indices file.")
    parser.add_argument("--out_dir", type=str, default='./data/cws-v2-data/model',
    ...
    parser.add_argument("--num_tags", type=int, default=150, help="BMES")
...
def main(unused_argv):
    ...
    vocab_size = check_vocab(hparams.vocab_file)  # 检查一下字典是否存在，并返回字典的字数
    hparams.add_hparam("vocab_size", vocab_size)

    if FLAGS.inference_output_file:
        # Inference
