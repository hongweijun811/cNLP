...
def train(hparams, model_creator):
    ...
    num_train_steps = hparams.num_train_steps  # 定义每次训练多少step 45000
    steps_per_stats = hparams.steps_per_stats  # 定义多少次训练后输出loss
    steps_per_external_eval = hparams.steps_per_external_eval  # 定义多少次训练后进行评估
    if not steps_per_external_eval:
        steps_per_external_eval = 10 * steps_per_stats    # 计算10次loss后评估结果
...
def _run_full_eval(hparams, loaded_train_model, train_sess, eval_model,
    ...
    score = evaluation(eval_model, model_dir, eval_sess,
               eval_iterator_feed_dict)
...
def _decode_by_function(writer_function, text_raw, decoded_tags, seq_lens):
    ...
    with open('sycws_folder/tag.pkl', 'rb') as f:
