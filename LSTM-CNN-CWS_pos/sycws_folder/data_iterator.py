...
def get_iterator(txt_dataset,
    ...
    txt_lb_dataset = tf.data.Dataset.zip((txt_dataset, lb_dataset))  # 得到一个包含X、y的数据集
    txt_lb_dataset = txt_lb_dataset.shuffle(output_buffer_size)      # 对数据集进行打乱

    txt_lb_dataset = txt_lb_dataset.map(
        lambda txt, lb: (
            tf.string_split([txt]).values, tf.string_split([lb]).values),
        num_parallel_calls=num_parallel_calls).prefetch(output_buffer_size)    # 拿出buffer_size大小的样本集
