...
def create_train_model(hparams, model_creator):
    txt_file = "%s.%s" % (hparams.train_prefix, "txt")  # train.txt
    lb_file = "%s.%s" % (hparams.train_prefix, "lb")  # train.lb
    vocab_file = hparams.vocab_file
    index_file = hparams.index_file  # ./sycws/indices.txt 分词的状态 0 1 2 3 共四个

    graph = tf.Graph()

    with graph.as_default(), tf.container("train"):
        vocab_table = lookup_ops.index_table_from_file(
            vocab_file, default_value=UNK_ID)    # 查找表，每个字对应一个token
        # for the labels
        index_table = lookup_ops.index_table_from_file(
            index_file, default_value=0)

        txt_dataset = tf.data.TextLineDataset(txt_file)    # 可以理解为训练集的X
        lb_dataset = tf.data.TextLineDataset(lb_file)      # 可以理解为训练集的y
