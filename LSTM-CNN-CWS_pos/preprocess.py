...
def clean_sentence(line_list):
    ...
    for token in line_list:
        div_id = token.rfind(DIVIDER)   # 找到分开词和词性的/的位置
        if div_id < 1:
...
def analyze_line(line_list, vob_dict):
    """
    将每个词中的字拆出来，并给每个字一个标签，S、B、M、E
    :param line_list: [词1，词2，词3，。。]
    :param vob_dict: 字典
    :return: charlist    [字1、字2、字3.。。]
              labellist   [字1的状态、字2的状态、字3的状态。。。]
    """
    char_list = []
    label_list = []
...
def generate_files(corpora, vob_path, char_file, train_word_file,
    ...
    tr_wd_wr = codecs.open(train_word_file, 'w', "utf-8")   # 训练集的X
    tr_lb_wr = codecs.open(train_label_file, 'w', "utf-8")  # 训练集的label
    ev_wd_wr = codecs.open(eval_word_file, 'w', "utf-8")    # 验证集的X
    ev_lb_wr = codecs.open(eval_label_file, 'w', "utf-8")   # 验证集的label

    ev_gold_wr = codecs.open(eval_gold_file, 'w', "utf-8")  # 验证集的分词 [词1、词2。。。]
    test_wr = codecs.open(test_file, 'w', "utf-8")    # 测试集
    gold_wr = codecs.open(gold_file, 'w', "utf-8")    # 分词结果
    ...
    with inp, ch_wr:
        for line in inp:
            ...
            write_line(char_list, ch_wr)  # 将语料的所有词拆分成一个个字
